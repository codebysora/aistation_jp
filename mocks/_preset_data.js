/**
 * Shared preset data + normalizer for model-select mocks (案A / 案B).
 * 元データは sample_data/preset.json と同一。先方サンプルが想定する
 * フラット `preset_list` 形式（jinja2 テンプレと同じキー名）へ整形する。
 */
(function (root) {
  var PRESETS_RAW = {
    "qwen3-0.6b-cpu": {
  ...
  /**
   * 入れ子構造の preset.json を partner の jinja テンプレが期待する
   * フラット形へ正規化（preset_list の 1 行と同等）。
   */
